...
async def upload_training(
    ...
    training_subtype: Optional[TrainingSubType] = Form(
        None, description="Spezifischer Trainingstyp"
    ),
    ...
    try:
        ...
        if not result["success"]:
            return TrainingUploadResponse(
                success=False, errors=result.get("errors", ["Unknown parsing error"])
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error processing training upload: {str(e)}"
        ) from e
